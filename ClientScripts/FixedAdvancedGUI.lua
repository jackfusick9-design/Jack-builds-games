-- FIXED ADVANCED GUI WITH WORKING SHOP BUTTON - Put in StarterPlayer > StarterCharacterScripts as LocalScript
...
shopButton.MouseButton1Click:Connect(function()
	print("[GUI] Shop button clicked")
	
	-- Wait for shop to exist
	local shop = Workspace:FindFirstChild("GardenShop")
	local attempts = 0
	while not shop and attempts < 50 do
		shop = Workspace:FindFirstChild("GardenShop")
		wait(0.1)
		attempts = attempts + 1
	end
	
	if shop then
		print("[GUI] Found shop, teleporting player")
		if player.Character then
			local shopPos = shop.Position + Vector3.new(0, 5, 0)
			player.Character:MoveTo(shopPos)
			
			local msg = Instance.new("Message")
			msg.Text = "🌱 Welcome to the Garden! Click bricks to buy them!"
			msg.Parent = Workspace
			game:GetService("Debris"):AddItem(msg, 4)
			print("[GUI] Teleported successfully")
		end
	else
		print("[GUI] ERROR: Shop not found!")
		local msg = Instance.new("Message")
		msg.Text = "ERROR: Garden shop not loaded yet!"
		msg.Parent = Workspace
